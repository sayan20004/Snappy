  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        {/* Email (Coming Soon) */}
        <button
          onClick={() => emailStatus?.connected ? setShowEmailModal(true) : setShowEmailConnect(true)}
          className={`card p-4 text-center hover:shadow-md transition-all border-2 border-dashed ${
        ...
        >
          <div className={`w-12 h-12 rounded-full ${
            emailStatus?.connected ? 'bg-blue-100' : 'bg-gray-100'
          } flex items-center justify-center mx-auto mb-2`}>
            <FiMail className={emailStatus?.connected ? 'text-blue-600' : 'text-gray-400'} size={20} />
          </div>
          <div className="text-xs font-medium text-gray-900 dark:text-gray-100">
            {emailStatus?.connected ? 'Fetch Emails' : 'Connect Email'}
          </div>
          <div className={`text-xs mt-1 ${emailStatus?.connected ? 'text-blue-600' : 'text-gray-400'}`}>
            {emailStatus?.connected ? `${emailStatus.email}` : 'Gmail, Outlook'}
          </div>
        </button>
      </div>
      ...
      <AnimatePresence>
        {showEmailConnect && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
            onClick={() => setShowEmailConnect(false)}
          >
            <motion.div
              initial={{ scale: 0.9 }}
              animate={{ scale: 1 }}
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    value={emailCredentials.provider}
                    onChange={(e) => setEmailCredentials({ ...emailCredentials, provider: e.target.value })}
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 text-sm text-blue-800 dark:text-blue-200">
                  <strong>Note:</strong> We never store your password. It's used only to establish a secure connection.
                </div>
              </div>
                  ...
                  onClick={async () => {
                    await connectEmail.mutateAsync({
                      ...
                      credentials: {
                        ...
                        appPassword: emailCredentials.appPassword,
                      },
                    });
                    setShowEmailConnect(false);
                    setEmailCredentials({ provider: 'gmail', email: '', appPassword: '' });
                  }}
                  disabled={!emailCredentials.email || !emailCredentials.appPassword || connectEmail.isPending}
                  className="btn btn-primary flex-1"
                >
                  {connectEmail.isPending ? 'Connecting...' : 'Connect Email'}
                </button>
                <button
                  onClick={() => {
                    setShowEmailConnect(false);
              ...
              <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    title="Refresh"
                  >
                    <FiUpload className={emailsLoading ? 'animate-spin' : ''} size={20} />
                  </button>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  Connected to {emailStatus?.email}
                </p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6">
                {emailsLoading ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">No recent emails found</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {emails.map((email) => (
                      <motion.div
                        ...
                        className="border-2 border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:border-blue-400 dark:hover:border-blue-500 transition-all cursor-pointer group"
                        onClick={() => {
                          processEmail.mutate(email);
      ...
      <div className="card p-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
        ...
        <div className="space-y-2 text-sm text-blue-800 dark:text-blue-200">
          <p>
            � <strong>Email:</strong> Connect your Gmail or Outlook, fetch emails, and select specific messages to generate tasks
          </p>
          <p>
